...
class Products extends ResourceController
{
    ...
    {
        $model = new ProductsModel();
        $name = trim($prod->part_brand_name).' '.$prod->stock_code.' '.(strlen($prod->alternative_name) > 2 ? $prod->alternative_name : $prod->name);
        $brandName = ($prod->part_brand_name ? $prod->part_brand_name : 'Diğer');
        $productSlug = aloparca::validUrl($brandName).aloparca::validUrl($name).aloparca::validUrl(str_replace('/','.',$prod->stock_code)).aloparca::validUrl($prod->product_no);
        $listPrice = number_format((float) $prod->list_price,2, '.', '');
        $salePrice = number_format((($prod->sale_price-$prod->discount_price+$prod->raise_price)*$prod->price_rate),2, '.', '');
        ...
        $arrRet = array(
            ...
            'name'                  => $name,
            'title'                 => $brandName.' - '.$name.' - '.(empty($prod->supplier_stock_code) ? str_replace('/','.',$prod->stock_code) : $prod->supplier_stock_code),
            'stock_code'            => str_replace('/','.',$prod->stock_code),
            'supplier_stock_code'   => $prod->supplier_stock_code,
            'brand_name'            => $brandName,
            'product_type'          => (int) ($prod->accessory_status == 1 ? 1 : $this->productTypeByBrand($brandName)),
            'stock_status'          => (int) $prod->stock_status,
            'free_shipping_status'  => (int) $prod->free_shipping_status,
            'list_price'            => (float) $listPrice,
            'sale_price'            => (float) $salePrice,
            'discount_rate'         => (float) number_format($this->getDiscountRate($listPrice, $salePrice),2),
            'image'                 => $prod->img_url,
        ...
        if($renderType == 'detail')
        {
            ...
            $description = $model->getProductDescription($prod->product_no, $prod->supplier_stock_code, $name, $brandName );
    ...
    public function createBreadCrumb($prod, $productSlug)
    {
        ...
        if($prod->oil_status == 1){
            ...
            $arrRet[2]['slug'] = 'madeni-yaglar/motor-yaglari/'.$oil->mainCatID.'/altkat'.aloparca::validUrl($oil->subCatName);
